...
# after step41, I want to know how to do transformer-transducer
...
# Function of this script  is used to prepared baseline model.
...
# no specaug
if [ ! -z $step11 ];then
    train_config=conf/transformer/train_hkust.yaml 
...
if [ ! -z $step14 ];then
    ...
    i=0; for pid in "${pids[@]}"; do wait ${pid} || ((++i)); done
    [ ${i} -gt 0 ] && echo "$0: ${i} background jobs are failed." && false
# cat log/run_bpe_baseline/steps13-18.log 
# dev_sge 26.1
# dev_man 18.9
fi
...
# pure ctc 
if [ ! -z $step15 ];then
    train_config=conf/transformer/train_hkust_pure_ctc.yaml
    preprocess_config=conf/specaug.yaml
    decode_config=conf/decode_pytorch_transformer_csj_v2_pure_ctc.yaml
    lm_config=conf/lm_default.yaml
    expname=${train_set}_${backend}_$(basename ${train_config%.*})_specaug
    expdir=$tgtdir/exp/${expname}
    mkdir -p ${expdir}
...
# pure attention
if [ ! -z $step17 ];then
    train_config=conf/transformer/train_hkust_pure_attention.yaml 
    preprocess_config=conf/specaug.yaml
    decode_config=conf/decode_pytorch_transformer_csj_v2_pure_attention.yaml
    lm_config=conf/lm_default.yaml
    expname=${train_set}_${backend}_$(basename ${train_config%.*})_specaug
    expdir=$tgtdir/exp/${expname}
    mkdir -p ${expdir}
...
# using big batchsize(e.g: 1498000 frames) , big epochs(e.g:100) and big accumulate_grad(e.g:8)
if [ ! -z $step19 ];then
...
# cuda memery:10011MB
# elapsed time 196585 seconds = 55 hours
fi
...
# deeper encoder(e.g: 36)  position_encode: using conv1d ,kernel 1
if [ ! -z $step21 ];then
...
# using bigger batchsize(1498000*2=2996000 frames) , big epochs(e.g:100) and big accumulate_grad(e.g:8)
if [ ! -z $step23 ];then
    train_config=conf/train_pytorch_transformer_lr5.0_ag8.v2_batchsize_bigger.yaml
    preprocess_config=conf/specaug.yaml
    decode_config=conf/decode_pytorch_transformer_csj_v2.yaml
    lm_config=conf/lm_default.yaml
    expname=${train_set}_${backend}_$(basename ${train_config%.*})_specaug
...
# cuda memery:  12487MiB 
# time : 147545 seconds = 41 hours 
fi
if [ ! -z $step24 ];then
    echo "LOG::: Decoding"
    train_config=conf/train_pytorch_transformer_lr5.0_ag8.v2_batchsize_bigger.yaml
...
# using bigger batchsize(1498000*4= 5992000 frames) , big epochs(e.g:100) and big accumulate_grad(e.g:8)
if [ ! -z $step25 ];then
    train_config=conf/train_pytorch_transformer_lr5.0_ag8.v2_batchsize_biggest.yaml
    preprocess_config=conf/specaug.yaml
    decode_config=conf/decode_pytorch_transformer_csj_v2.yaml
    lm_config=conf/lm_default.yaml
    expname=${train_set}_${backend}_$(basename ${train_config%.*})_specaug
...
# cuda memery: 17991MiB
# time :   124340 seconds = 34.5 hours 
fi
if [ ! -z $step26 ];then
    echo "LOG::: Decoding"
    train_config=conf/train_pytorch_transformer_lr5.0_ag8.v2_batchsize_biggest.yaml
...
# using bigger batchsize(1498000 *3 =4494000 frames) , big epochs(e.g:120) and big accumulate_grad(e.g:4)
if [ ! -z $step27 ];then
    train_config=conf/transformer/train_pytorch_transformer_large_lr10_ag4.yaml 
    preprocess_config=conf/specaug.yaml
    decode_config=conf/decode_pytorch_transformer_csj_v2.yaml
    lm_config=conf/lm_default.yaml
    expname=${train_set}_${backend}_$(basename ${train_config%.*})_specaug
...
# using bigger batchsize(1498000 *3 =4494000 frames) , big epochs(e.g:120) and big accumulate_grad(e.g:2)
if [ ! -z $step29 ];then
    train_config=conf/transformer/train_pytorch_transformer_large_lr10_ag2.yaml
    preprocess_config=conf/specaug.yaml
    decode_config=conf/decode_pytorch_transformer_csj_v2.yaml
    lm_config=conf/lm_default.yaml
    expname=${train_set}_${backend}_$(basename ${train_config%.*})_specaug
...
# conf/train_pytorch_transformer_lr5.0_ag8.v2_batchsize_bigger_head6.yaml

# using bigger batchsize(1498000*2=2996000 frames) , big epochs(e.g:100) and big accumulate_grad(e.g:8),heads from 4 to 8
if [ ! -z $step31 ];then
    train_config=conf/train_pytorch_transformer_lr5.0_ag8.v2_batchsize_bigger_head8.yaml
    preprocess_config=conf/specaug.yaml
    decode_config=conf/decode_pytorch_transformer_csj_v2.yaml
    lm_config=conf/lm_default.yaml
    expname=${train_set}_${backend}_$(basename ${train_config%.*})_specaug
...
# using big beam size(e.g: 40) to decoder
if [ ! -z $step33 ];then
    echo "LOG::: Decoding"
    train_config=conf/train_pytorch_transformer_lr5.0_ag8.v2_batchsize_bigger.yaml
    preprocess_config=conf/specaug.yaml
    decode_config=conf/decode_pytorch_transformer_csj_v2_beamsize40.yaml
    lm_config=conf/lm_default.yaml
    expname=${train_set}_${backend}_$(basename ${train_config%.*})_specaug
    expdir=$tgtdir/exp/${expname}
    [ -d $expdir ] || mkdir -p ${expdir}
...
# using big beam size(e.g: 60) to decoder
if [ ! -z $step34 ];then
    echo "LOG::: Decoding"
    train_config=conf/train_pytorch_transformer_lr5.0_ag8.v2_batchsize_bigger.yaml
    preprocess_config=conf/specaug.yaml
    decode_config=conf/decode_pytorch_transformer_csj_v2_beamsize60.yaml
    lm_config=conf/lm_default.yaml
    expname=${train_set}_${backend}_$(basename ${train_config%.*})_specaug
    expdir=$tgtdir/exp/${expname}
    [ -d $expdir ] || mkdir -p ${expdir}
...
# using big beam size(e.g: 80) to decoder
if [ ! -z $step35 ];then
    echo "LOG::: Decoding"
    train_config=conf/train_pytorch_transformer_lr5.0_ag8.v2_batchsize_bigger.yaml
    preprocess_config=conf/specaug.yaml
    decode_config=conf/decode_pytorch_transformer_csj_v2_beamsize80.yaml
    lm_config=conf/lm_default.yaml
    expname=${train_set}_${backend}_$(basename ${train_config%.*})_specaug
    expdir=$tgtdir/exp/${expname}
    [ -d $expdir ] || mkdir -p ${expdir}
...
# using big beam size(e.g: 100) to decoder
if [ ! -z $step36 ];then
    echo "LOG::: Decoding"
    train_config=conf/train_pytorch_transformer_lr5.0_ag8.v2_batchsize_bigger.yaml
    preprocess_config=conf/specaug.yaml
    decode_config=conf/decode_pytorch_transformer_csj_v2_beamsize100.yaml
    lm_config=conf/lm_default.yaml
    expname=${train_set}_${backend}_$(basename ${train_config%.*})_specaug
    expdir=$tgtdir/exp/${expname}
    [ -d $expdir ] || mkdir -p ${expdir}
...
# now  By changing the beamsize, we found almost no impact, only when beamsize=40, it has a little better than other.
# I will to do two experiments, 
# fistly , I explore ctc type(: they are warpctc, builtin pytorch1.3.1) impact
# using bigger batchsize(1498000 *3 =4494000 frames) , big epochs(e.g:120) and big accumulate_grad(e.g:4)
# note: ctc-type : from warpctc to builtin
#       pytorch version : from 1.0.1 to 1.3.1
if [ ! -z $step37 ];then
    train_config=conf/transformer/train_pytorch_transformer_large_lr10_ag4_builtin_ctc.yaml
    preprocess_config=conf/specaug.yaml
    decode_config=conf/decode_pytorch_transformer_csj_v2.yaml
    lm_config=conf/lm_default.yaml
    expname=${train_set}_${backend}_$(basename ${train_config%.*})_specaug
...
# # using bigger batchsize(1498000 *3 =4494000 frames) , big epochs(e.g:120) and big accumulate_grad(e.g:4)
# now secondly , I used beamsize=40, to decoer.
if [ ! -z $step39 ];then
    echo "LOG::: Decoding"
    train_config=conf/transformer/train_pytorch_transformer_large_lr10_ag4.yaml
    preprocess_config=conf/specaug.yaml
...
# This configure is best for seame in espnet1 version 
if [ ! -z $step40 ];then
    
    train_config=conf/transformer/train_pytorch_transformer_large_lr10_ag6.yaml
    preprocess_config=conf/specaug.yaml
    decode_config=conf/decode_pytorch_transformer_csj_v2_beamsize40.yaml
    lm_config=conf/lm_default.yaml
    expname=${train_set}_${backend}_$(basename ${train_config%.*})_specaug
...
# 2020-3-21
# from here, I want to know how to do train transducer.
if [ ! -z $step42 ];then
    [ -f ./path_v10.sh ] && . ./path_v10.sh
    #train_config=conf/transducer/train_transducer.yaml # out of CUDA memery
    train_config=conf/transducer/train_transformer_transducer.yaml # 3 12GPU is not enough 
    preprocess_config=conf/specaug.yaml
...
if [ ! -z $step43 ];then
    echo "LOG::: Decoding"
    [ -f ./path_v10.sh ] && . ./path_v10.sh
    train_config=conf/transducer/train_transformer_transducer.yaml
    preprocess_config=conf/specaug.yaml
    decode_config=conf/transducer/decode_transducer.yaml
    lm_config=conf/lm_default.yaml
    expname=${train_set}_${backend}_$(basename ${train_config%.*})_specaug
