...
# 1. using speed perturbed method
# 2. data augmentation (four kind of noise : clean = 1: 1)
...
if [ ! -z $step04 ]; then
     ...
     utils/subset_data_dir_tr_cv.sh  --cv-spk-percent 10 $tgtdir/data/train_aug $tgtdir/data/train_aug_trn $tgtdir/data/${train_dev} || exit 1;
...
if [ ! -z $step07 ];then
    ...
    cut -f 2- -d" " ${tgtdir}/data/train_aug_trn_sp/text>${tgtdir}/data/lang_char_aug/input.txt
...
if [ ! -z $step09 ]; then
   ...
   lmdict=${dict}
   mkdir -p ${lmdatadir}
    cut -f 2- -d" "  $tgtdir/data/train_aug_trn_sp/text | \
