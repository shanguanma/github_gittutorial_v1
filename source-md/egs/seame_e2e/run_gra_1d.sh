...
# 1. data augmentation (four kind of noise : clean = 1: 1)
...
tgtdir=run_gra_1d
train_set=train_aug_trn
train_dev=train_aug_dev
...
if [ ! -z $step01 ];then
    ...
    # copy data augmentation (four kind of noise : clean = 1: 1)
    # more detail .you can see /home3/md510/w2019a/kaldi-recipe/add-noise-to-seame-v18/run.sh 
    cp -r  /home3/md510/w2019a/kaldi-recipe/add-noise-to-seame-v18/data/train_aug  $tgtdir/data/
    utils/validate_data_dir.sh $tgtdir/data/train_aug
fi
...
add_four_noise_data_dir=/home4/md510/w2019a/kaldi-recipe/add-many-method/data_1/train_new_aug
if [ ! -z $step03 ];then
   utils/subset_data_dir.sh $add_four_noise_data_dir 93938 ${tgtdir}/data/train_new_aug_sub
   fbankdir=fbank
   for x in train_new_aug_sub; do
        steps/make_fbank_pitch.sh --cmd "$train_cmd" --nj 40 --write_utt2num_frames true \
            ${tgtdir}/data/${x} ${tgtdir}/exp/make_fbank/${x} ${tgtdir}/${fbankdir}
...
if [ ! -z $step04 ]; then
     ...
     utils/combine_data.sh $tgtdir/data/train_aug  ${tgtdir}/data/train_new_aug_sub ${tgtdir}/data/train
    
     echo "LOG:::make a train_dev and train_trn from train set"
     # train_trn : train_dev =90:10
     utils/subset_data_dir_tr_cv.sh  --cv-spk-percent 10 $tgtdir/data/train_aug $tgtdir/data/${train_set} $tgtdir/data/${train_dev} || exit 1;
#
#     # speed-perturbed for train_trn set
#     echo "LOG:::make speed-perturbed for train_trn set "
#     utils/perturb_data_dir_speed.sh 0.9  ${tgtdir}/data/train_aug_trn  ${tgtdir}/data/temp1
#     utils/perturb_data_dir_speed.sh 1.0  ${tgtdir}/data/train_aug_trn  ${tgtdir}/data/temp2
#     utils/perturb_data_dir_speed.sh 1.1  ${tgtdir}/data/train_aug_trn  ${tgtdir}/data/temp3
...
# ####  now I using train_aug_trn_sp text to get dictionary and train a lm.
########
dict=${tgtdir}/data/lang_char_aug/${train_set}_${bpemode}${nbpe}_units.txt
bpemodel=${tgtdir}/data/lang_char_aug/${train_set}_${bpemode}${nbpe}
echo "dictionary: ${dict}"

if [ ! -z $step07 ];then
    ### Task dependent. You have to check non-linguistic symbols used in the corpus.
    echo "LOG:::stage 6: Dictionary  Preparation"
    mkdir -p ${tgtdir}/data/lang_char_aug/

    echo "LOG:::in order to make a dictionary"
    echo "<unk> 1">${dict} #<unk> must be 1, 0 will be used for "blank" in CTC.
    cut -f 2- -d" " ${tgtdir}/data/${train_set}/text>${tgtdir}/data/lang_char_aug/input.txt
    #cut -f 2- ${tgtdir}/data/train/text>${tgtdir}/data/lang_char/input.txt
    spm_train --input=${tgtdir}/data/lang_char_aug/input.txt --vocab_size=${nbpe} --model_type=${bpemode} --model_prefix=${bpemodel} --input_sentence_size=100000000
    spm_encode --model=${bpemodel}.model --output_format=piece<${tgtdir}/data/lang_char_aug/input.txt | tr ' ' '\n'| sort | uniq | awk '{print $0 " " NR+1}'>>${dict}
    wc -l ${dict}
...
if [ ! -z $step09 ]; then
   ...
   lmdatadir=$tgtdir/data/local_aug/lm_train_${bpemodel}
   lmdict=${dict}
   mkdir -p ${lmdatadir}
    cut -f 2- -d" "  $tgtdir/data/${train_set}/text | \
