...
# the script is demo. I only use two wave file as test set.
# then spilt the test set into two part on average.
# v4 is same as v2, but I used the mandarin data(e.g:/data/users/hhx502/ldc-cts2016/hkust-mandarin-tele-trans-ldc2005t32/data/local/data/train/) 
# to train PLDA model. in other words, I use this mandarin data to instead of ser_2004_2008
...
if [ ! -z $step01 ];then
   ...
   internal_folder_small=data/test-data-20190808-tmp_demo_1
   mkdir -p $internal_folder_small
   head -n 2 $internal_folder/wav-list.txt > $internal_folder_small/wav-list.txt
...
if [ ! -z $step02 ];then
  ...
  internal_folder_small=data/test-data-20190808-tmp_demo_1
  data=data/test-data-20190808-kaldi_demo_1
  mkdir -p $data
...
#data=data/test-data-2019080-kaldi_demo_1
if [ ! -z $step03 ]; then
  internal_folder=data/test-data-20190808-tmp
  data=data/test-data-20190808-kaldi_demo_1
  ...
  internal_folder_small=data/test-data-20190808-tmp_demo_1
  head -n 2 $internal_folder/text-grid-list.txt > $internal_folder_small/text-grid-list.txt
...
# make two subdataset 
test_data_dir=data/test-data-20190808-kaldi_demo_1
...
if [ ! -z $step13 ];then
  ...
  for name in  test-data-20190808-kaldi_demo_1 test-data-20190808-kaldi_demo_1_1 test-data-20190808-kaldi_demo_1_2; do
  ...
  for name in  test-data-20190808-kaldi_demo_1_1 test-data-20190808-kaldi_demo_1_2 ; do
...
# Extract x-vectors
sub_test_data_1=test-data-20190808-kaldi_demo_1_1
sub_test_data_2=test-data-20190808-kaldi_demo_1_2
...
test_data=test-data-20190808-kaldi_demo_1
...
if [ ! -z $step18 ]; then
  ...
  der=$(grep -oP 'DIARIZATION\ ERROR\ =\ \K[0-9]+([.][0-9]+)?' \
    $nnet_dir/results_${test_data}_mandarin_supervised/DER_threshold.txt)
  # Using supervised calibration, DER:3.09
  # path:exp/ellerao_retrain_xvector_model/results_test-data-20190808-kaldi_demo_1_supervised/DER_threshold.txt
...
if [ ! -z $step19 ]; then
  ...
  der=$(grep -oP 'DIARIZATION\ ERROR\ =\ \K[0-9]+([.][0-9]+)?' \
    $nnet_dir/results_${test_data}_mandarin_oracle/DER_num_spk.txt)
  # Using the oracle number of speakers, DER:11.51%
  # exp/ellerao_retrain_xvector_model/results_test-data-20190808-kaldi_demo_1_oracle/DER_num_spk.txt 
