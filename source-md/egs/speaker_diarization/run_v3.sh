...
# the script is demo. I only use four wave file as test set.
...
if [ ! -z $step01 ];then
   ...
   internal_folder_small=data/test-data-20190808-tmp_demo_2
   mkdir -p $internal_folder_small
   head -n 4 $internal_folder/wav-list.txt > $internal_folder_small/wav-list.txt
...
if [ ! -z $step02 ];then
  ...
  internal_folder_small=data/test-data-20190808-tmp_demo_2
  data=data/test-data-20190808-kaldi_demo_2
  mkdir -p $data
...
if [ ! -z $step03 ]; then
  ...
  internal_folder_small=data/test-data-20190808-tmp_demo_2
  head -n $select_number $internal_folder/text-grid-list.txt > $internal_folder_small/text-grid-list.txt
...
# make two subdataset 
test_data_dir=data/test-data-20190808-kaldi_demo_2
...
if [ ! -z $step13 ];then
  ...
  for name in  test-data-20190808-kaldi_demo_2 test-data-20190808-kaldi_demo_2_1 test-data-20190808-kaldi_demo_2_2; do
  ...
  for name in  test-data-20190808-kaldi_demo_2_1 test-data-20190808-kaldi_demo_2_2 ; do
...
# Extract x-vectors
sub_test_data_1=test-data-20190808-kaldi_demo_2_1
sub_test_data_2=test-data-20190808-kaldi_demo_2_2
...
test_data=test-data-20190808-kaldi_demo_2
...
if [ ! -z $step18 ]; then
  ...
  der=$(grep -oP 'DIARIZATION\ ERROR\ =\ \K[0-9]+([.][0-9]+)?' \
    $nnet_dir/results_${test_data}_supervised/DER_threshold.txt)
  # Using supervised calibration, DER:4.09%
  # path: exp/ellerao_retrain_xvector_model/results_test-data-20190808-kaldi_demo_2_supervised/DER_threshold.txt 
