...
# v6 is same as v5, I use sre_2004_2008 instead of hkust-mandarin to train PLDA model.
...
# make two subdataset 
test_data_dir=data/test-data-20190808-kaldi_entire_${version}
...
# make feature(e.g.:mfcc(23 dim),vad,cmn) from sre_2004_2008
# sre_2004_2008 is used to train plda model.
if [ ! -z $step12 ];then
...
if [ ! -z $step13 ];then
  ...
  for name in  test-data-20190808-kaldi_entire_${version} test-data-20190808-kaldi_entire_${version}_1 test-data-20190808-kaldi_entire_${version}_2; do
    steps/make_mfcc.sh --mfcc-config conf/mfcc.conf --nj 40 \
      --cmd "$train_cmd" --write-utt2num-frames true \
      data/$name exp/make_mfcc $mfccdir
    utils/fix_data_dir.sh data/$name
  ...
  for name in  test-data-20190808-kaldi_entire_${version}_1 test-data-20190808-kaldi_entire_${version}_2 ; do
    sid/compute_vad_decision.sh --nj 40 --cmd "$train_cmd" \
      data/$name exp/make_vad $vaddir
    utils/fix_data_dir.sh data/$name
  done
...
# Extract x-vectors
sub_test_data_1=test-data-20190808-kaldi_entire_${version}_1
sub_test_data_2=test-data-20190808-kaldi_entire_${version}_2
...
if [ ! -z $step15 ]; then
  ...
  diarization/nnet3/xvector/extract_xvectors.sh --cmd "$train_cmd --mem 5G" \
    ...
    data/${sub_test_data_1}_cmn $nnet_dir/xvectors_${sub_test_data_1}_${train_PLDA_set_name}

  diarization/nnet3/xvector/extract_xvectors.sh --cmd "$train_cmd --mem 5G" \
    --nj 10 --window 1.5 --period 0.75 --apply-cmn false \
    --min-segment 0.5 $nnet_dir \
    data/${sub_test_data_2}_cmn $nnet_dir/xvectors_${sub_test_data_2}_${train_PLDA_set_name}
  ...
  diarization/nnet3/xvector/extract_xvectors.sh --cmd "$train_cmd --mem 10G" \
    --nj 1 --window 3.0 --period 10.0 --min-segment 1.5 --apply-cmn false \
...
if [ ! -z $step16 ]; then
  ...
  $train_cmd $nnet_dir/xvectors_${sub_test_data_1}_${train_PLDA_set_name}/log/plda.log \
    ivector-compute-plda ark:$nnet_dir/xvectors_${train_PLDA_set_name}_segmented_128k/spk2utt \
      ...
      | transform-vec $nnet_dir/xvectors_${sub_test_data_1}_${train_PLDA_set_name}/transform.mat ark:- ark:- \
      | ivector-normalize-length ark:- ark:- |" \
    $nnet_dir/xvectors_${sub_test_data_1}_${train_PLDA_set_name}/plda || exit 1;
  ...
  $train_cmd $nnet_dir/xvectors_${sub_test_data_2}_${train_PLDA_set_name}/log/plda.log \
    ivector-compute-plda ark:$nnet_dir/xvectors_${train_PLDA_set_name}_segmented_128k/spk2utt \
      "ark:ivector-subtract-global-mean \
      scp:$nnet_dir/xvectors_${train_PLDA_set_name}_segmented_128k/xvector.scp ark:- \
      | transform-vec $nnet_dir/xvectors_${sub_test_data_2}_${train_PLDA_set_name}/transform.mat ark:- ark:- \
      | ivector-normalize-length ark:- ark:- |" \
    $nnet_dir/xvectors_${sub_test_data_2}_${train_PLDA_set_name}/plda || exit 1;
fi
...
if [ ! -z $step17 ]; then
  ...
  diarization/nnet3/xvector/score_plda.sh --cmd "$train_cmd --mem 4G" \
    --nj 1 $nnet_dir/xvectors_${sub_test_data_2}_${train_PLDA_set_name} $nnet_dir/xvectors_${sub_test_data_1}_${train_PLDA_set_name} \
    $nnet_dir/xvectors_${sub_test_data_1}_${train_PLDA_set_name}/plda_scores

  # Do the same thing for ${sub_test_data_2}.
  diarization/nnet3/xvector/score_plda.sh --cmd "$train_cmd --mem 4G" \
    --nj 1 $nnet_dir/xvectors_${sub_test_data_1}_${train_PLDA_set_name} $nnet_dir/xvectors_${sub_test_data_2}_${train_PLDA_set_name} \
    $nnet_dir/xvectors_${sub_test_data_2}_${train_PLDA_set_name}/plda_scores
fi

test_data=test-data-20190808-kaldi_entire_${version}
...
if [ ! -z $step18 ]; then
  ...
  for dataset in ${sub_test_data_1} ${sub_test_data_2}; do
    ...
    for threshold in -0.3 -0.2 -0.1 -0.05 0 0.05 0.1 0.2 0.3; do
      diarization/cluster.sh --cmd "$train_cmd --mem 4G" --nj 1 \
        --threshold $threshold $nnet_dir/xvectors_${dataset}_${train_PLDA_set_name}/plda_scores \
        $nnet_dir/xvectors_${dataset}_${train_PLDA_set_name}/plda_scores_t$threshold

      md-eval.pl -1 -c 0.25 -r data/$dataset/ref.rttm \
       -s $nnet_dir/xvectors_${dataset}_${train_PLDA_set_name}/plda_scores_t$threshold/rttm \
       2> $nnet_dir/tuning/${dataset}_${train_PLDA_set_name}_t${threshold}.log \
       > $nnet_dir/tuning/${dataset}_${train_PLDA_set_name}_t${threshold}
    ...
    echo "$best_threshold" > $nnet_dir/tuning/${dataset}_${train_PLDA_set_name}_best
  done
  ...
  diarization/cluster.sh --cmd "$train_cmd --mem 4G" --nj 1 \
    --threshold $(cat $nnet_dir/tuning/${sub_test_data_2}_${train_PLDA_set_name}_best) \
    $nnet_dir/xvectors_${sub_test_data_1}_${train_PLDA_set_name}/plda_scores $nnet_dir/xvectors_${sub_test_data_1}_${train_PLDA_set_name}/plda_scores

  # Do the same thing for ${sub_test_data_2}, treating ${sub_test_data_1} as a held-out dataset
  # to discover a stopping threshold.
  diarization/cluster.sh --cmd "$train_cmd --mem 4G" --nj 1 \
    --threshold $(cat $nnet_dir/tuning/${sub_test_data_1}_${train_PLDA_set_name}_best) \
    $nnet_dir/xvectors_${sub_test_data_2}_${train_PLDA_set_name}/plda_scores $nnet_dir/xvectors_${sub_test_data_2}_${train_PLDA_set_name}/plda_scores

  mkdir -p $nnet_dir/results_${test_data}_${train_PLDA_set_name}_supervised
  # Now combine the results for ${sub_test_data_1} and ${sub_test_data_1} and evaluate it
  # together.
  cat $nnet_dir/xvectors_${sub_test_data_1}_${train_PLDA_set_name}/plda_scores/rttm \
    $nnet_dir/xvectors_${sub_test_data_2}_${train_PLDA_set_name}/plda_scores/rttm | md-eval.pl -1 -c 0.25 -r \
    ${test_data_dir}/fullref.rttm -s - 2> $nnet_dir/results_${test_data}_${train_PLDA_set_name}_supervised/threshold.log \
    > $nnet_dir/results_${test_data}_${train_PLDA_set_name}_supervised/DER_threshold.txt
  der=$(grep -oP 'DIARIZATION\ ERROR\ =\ \K[0-9]+([.][0-9]+)?' \
    $nnet_dir/results_${test_data}_${train_PLDA_set_name}_supervised/DER_threshold.txt)
  # Using supervised calibration, DER:6.33%
  # path:cat exp/ellerao_retrain_xvector_model/results_test-data-20190808-kaldi_entire_v6_sre_2004_2008_supervised/DER_threshold.txt
...
if [ ! -z $step19 ]; then
  ...
  diarization/cluster.sh --cmd "$train_cmd --mem 4G" --nj 1 \
    ...
    $nnet_dir/xvectors_${sub_test_data_1}_${train_PLDA_set_name}/plda_scores $nnet_dir/xvectors_${sub_test_data_1}_${train_PLDA_set_name}/plda_scores_num_spk

  diarization/cluster.sh --cmd "$train_cmd --mem 4G" --nj 1 \
    --reco2num-spk data/${sub_test_data_2}/reco2num_spk \
    $nnet_dir/xvectors_${sub_test_data_2}_${train_PLDA_set_name}/plda_scores $nnet_dir/xvectors_${sub_test_data_2}_${train_PLDA_set_name}/plda_scores_num_spk

  mkdir -p $nnet_dir/results_${test_data}_${train_PLDA_set_name}_oracle
  # Now combine the results for ${sub_test_data_1} and ${sub_test_data_2} and evaluate it together.
  cat $nnet_dir/xvectors_${sub_test_data_1}_${train_PLDA_set_name}/plda_scores_num_spk/rttm \
  $nnet_dir/xvectors_${sub_test_data_2}_${train_PLDA_set_name}/plda_scores_num_spk/rttm \
    | md-eval.pl -1 -c 0.25 -r ${test_data_dir}/fullref.rttm -s - 2> $nnet_dir/results_${test_data}_${train_PLDA_set_name}_oracle/num_spk.log \
    > $nnet_dir/results_${test_data}_${train_PLDA_set_name}_oracle/DER_num_spk.txt
  der=$(grep -oP 'DIARIZATION\ ERROR\ =\ \K[0-9]+([.][0-9]+)?' \
    $nnet_dir/results_${test_data}_${train_PLDA_set_name}_oracle/DER_num_spk.txt)
