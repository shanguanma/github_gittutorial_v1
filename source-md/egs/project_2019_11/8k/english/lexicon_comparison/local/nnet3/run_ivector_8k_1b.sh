...
if [ "$speed_perturb" == "true" ]; then
  if [ $stage -le 1 ]; then
    ...
    for datadir in ${train_set} ${ivector_train_set}; do
      utils/data/perturb_data_dir_speed_3way.sh $tgtdir/data/${datadir} $tgtdir/data/${datadir}_sp
      utils/fix_data_dir.sh $tgtdir/data/${datadir}_sp
 
      mfccdir=$tgtdir/mfcc_perturbed
      steps/make_mfcc.sh --cmd "$cmd" --nj 40 --mfcc-config conf/mfcc_8k.conf \
        $tgtdir/data/${datadir}_sp $exp_root/make_mfcc/${datadir}_sp $mfccdir || exit 1;
...
if [ $stage -le 3 ]; then
  ...
  for dataset in $ivector_train_set $train_set; do
    ...
    steps/make_mfcc.sh --nj 40 --mfcc-config conf/mfcc_hires_8k.conf \
        --cmd "$cmd" $tgtdir/data/${dataset}_hires $exp_root/make_hires/$dataset $mfccdir;
    steps/compute_cmvn_stats.sh $tgtdir/data/${dataset}_hires $exp_root/make_hires/${dataset} $mfccdir;
  ...
  for dataset in $test_set_1 $test_set_2; do
    ...
    steps/make_mfcc.sh --cmd "$cmd" --nj 10 --mfcc-config conf/mfcc_hires_8k.conf \
        $tgtdir/data/${dataset}_hires $exp_root/make_hires/$dataset $mfccdir;
    steps/compute_cmvn_stats.sh $tgtdir/data/${dataset}_hires $exp_root/make_hires/$dataset $mfccdir;
...
if [ $stage -le 6 ]; then
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$cmd" --nj 10 \
    $tgtdir/data/${ivector_train_set}_hires $exp_root/nnet3${nnet3_affix}/diag_ubm \
...
if [ $stage -le 7 ]; then
  ...
  utils/data/modify_speaker_info.sh --utts-per-spk-max 2 \
    $tgtdir/data/${ivector_train_set}_hires $tgtdir/data/${ivector_train_set}_max2_hires

  steps/online/nnet2/extract_ivectors_online.sh --cmd "$cmd" --nj 30 \
    $tgtdir/data/${ivector_train_set}_max2_hires $exp_root/nnet3${nnet3_affix}/extractor \
...
if [ $stage -le 8 ]; then
  for dataset in $test_set_1 $test_set_2; do
    steps/online/nnet2/extract_ivectors_online.sh --cmd "$cmd" --nj 10 \
      $tgtdir/data/${dataset}_hires $exp_root/nnet3${nnet3_affix}/extractor \
