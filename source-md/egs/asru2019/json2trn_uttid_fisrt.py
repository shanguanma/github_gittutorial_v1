...
def convert(jsonf, dic, refs, hyps, num_spkrs=1):
    ...
    for ns in range(num_spkrs):
        ...
        for x in j['utts']:
            ...
            # official hyp_file format: <utterance> <utt2spk-uttid> 
            #                            hamburger manuel 的 替 代 方 案(G0476-G0476T0392G0476S01010001) 
            #hyp_file.write(" ".join(seq).replace('<eos>', '')),
            #hyp_file.write(" (" + j['utts'][x]['utt2spk'].replace('-', '_') + "-" + x + ")\n")

            # md hyp_file format: <utt2spk-uttid> <utterance>
            #                     G0476-G0476T0392G0476S01010001 hamburger manuel 的 替 代 方 案
            hyp_file.write(j['utts'][x]['utt2spk'].replace('-', '_') + "-" + x + "\t"),
            hyp_file.write(" ".join(seq).replace('<eos>', '')+ '\n')
            # reference
            if num_spkrs == 1:
            ...
            # official ref_file format: <utterance> <utt2spk-uttid>
            #                           hamburger menu 的 替 代 方 案(G0476-G0476T0392G0476S01010001)
            #ref_file.write(seq + " (" + j['utts'][x]['utt2spk'].replace('-', '_') + "-" + x + ")\n")

            # md ref_file format: <utt2spk-uttid> <utterance>
            #                     G0476-G0476T0392G0476S01010001 hamburger menu 的 替 代 方 案                                    
            ref_file.write(j['utts'][x]['utt2spk'].replace('-', '_') + "-" + x + '\t' + seq + "\n")
