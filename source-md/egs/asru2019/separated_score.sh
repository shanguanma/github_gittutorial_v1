...
if [ ! -z $step02 ];then
  if [ $num_spkrs -eq 1 ]; then
    source-md/egs/asru2019/json2trn_uttid_fisrt.py ${dir}/data_new.json ${dict} --num-spkrs ${num_spkrs} --refs ${dir}/ref_new.trn --hyps ${dir}/hyp_new.trn
  fi
...
# 3.get word hyp and word ref
if [ ! -z $step03 ];then
    if [ -n "$bpe" ];then
        spm_decode --model=${bpemodel} --input_format=piece < ${dir}/ref_new.trn | sed -e "s/▁/ /g" > ${dir}/ref_new.wrd.trn
        spm_decode --model=${bpemodel} --input_format=piece < ${dir}/hyp_new.trn | sed -e "s/▁/ /g" > ${dir}/hyp_new.wrd.trn
    fi
fi
# 4. computer wer
if [ ! -z $step04 ];then
   source-md/egs/asru2019/compute-wer.py   ${dir}/ref_new.wrd.trn ${dir}/hyp_new.wrd.trn
