...
def get_parse():
    parser = argparse.ArgumentParser(description="make grapheme lexicon.txt")
    parser.add_argument(
        "--wordlist_en",
        type=str,
        required=True,
...
def BuildGraphemeLexicon(
    wordlist_file: Iterable[Union[str, Path]], dict_dir: Union[str, Path]
):
    dict_dir = Path(dict_dir)
    with open(wordlist_file, "r", encoding="utf-8") as f, open(
        dict_dir / f"pre_lexicon.txt", "w", encoding="utf-8"
    ) as fo:
        for line in f:
            ...
            if len(line) == 2:
                ...
                raise RuntimeError(
                    f"Not supported: {len(line)}, only supported single row or two rows text file"
                )
            if len(word) == 1:
...
def creat_lexicon(dict_dir: Union[str, Path]):
    ...
    with open(pre_dict, "r", encoding="utf-8") as fi:
        ...
        lex_dict = {
            key: value for key, value in lex_dict.items() if not key.startswith("#")
        }
