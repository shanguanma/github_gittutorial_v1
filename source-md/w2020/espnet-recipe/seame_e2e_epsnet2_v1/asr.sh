...
asr_stats_dir="${expdir}/asr_stats_$(basename "${asr_config}" .yaml)_${feats_type}_${token_type}"
lm_stats_dir="${expdir}/lm_stats_$(basename "${lm_config}" .yaml)_${lm_token_type}"
    ...
    if [ -n "${speed_perturb_factors}" ]; then
       ...
       for factor in ${speed_perturb_factors}; do
           if [[ $(bc <<<"${factor} != 1.0") == 1 ]]; then
               utils/perturb_data_dir_speed.sh "${factor}" "data/${train_set}" "data/${train_set}_sp${factor}"
