...
hmm_gmm_version=v1  # when hmm_gmm_version=v1, it use lda_mllt without splice frame to train tri2 system
                    #  when hmm_gmm_version=v2, it use delta+delta-delta  feature to train tri2 system. 
...
Options:
    ...
    --use_pp                     # whether we compute the pronunciation and silence probabilities from training data, default is true, (default="${use_pp}")
                                 # and re-create the lang and lang_test directory.
    # realted gmm 
    --shortest_utt_num           # these utterances is used to do train mono, it is useful for alignment. this value(default="$shortest_utt_num")
    ...
    --dnn_size                   # if train data less than about 50  hours ,it will be "small", else , it will normal. (default="${dnn_size}")
    --hmm_gmm_version            # when hmm_gmm_version=v1, it use lda_mllt without splice frame to train tri2 system
                                 # when hmm_gmm_version=v2, it use delta+delta-delta  feature to train tri2 system. (default="${hmm_gmm_version}")
