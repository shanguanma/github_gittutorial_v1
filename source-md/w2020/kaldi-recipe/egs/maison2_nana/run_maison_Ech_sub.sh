...
# realted gmm 
shortest_utt_num=9000  # these utterances is used to do train mono, it is useful for alignment. this value
...
dnn_size=small       # if train data less 100  hours ,it will be "small", else , it will "normal".


# [Task dependent] Set the datadir name created by
src_train_data_dir=data/model_ch_sub_all
src_test_data_dir=data/dev
train_set=data_sub_Ech                  # train set name
...
tgtdir=train_data_Ech_sub_new       #  root folder of the entire asr system 
suffix="_Ech"               # you can set it, then run different model in same tgtdir,
