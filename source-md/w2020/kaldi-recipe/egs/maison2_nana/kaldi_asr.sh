...
if [ ${stage} -le 9 ] && [ ${stop_stage} -ge 9 ];then
  ...
  utils/subset_data_dir.sh --shortest  $tgtdir/data$suffix/${train_set} ${shortest_utt_num}  $tgtdir/data$suffix/${train_set}_${shortest_utt_num}short 
fi     
...
dir=$exp_root/chain${nnet3_affix}/cnn_tdnn${nnet3_affix}${sp_suffix}   # chain modle ouput path
...
if [ ${stage} -le 27 ] && [ ${stop_stage} -ge 27 ];then
   log "Stage 26: decode test set offline ...."
   if $use_pp ;then
     for decode_set in $test_sets; do
      #_nj=$(min "${nj}" "$(<"$tgtdir/data$suffix/${decod_set}/utt2spk" wc -l)")
...
if [ ${stage} -le 28 ] && [ ${stop_stage} -ge 28 ];then
 log " Stage 27 : make wer score for test set..."
 if $use_pp ;then
   for decode_set in $test_sets; do
...
if [ ${stage} -le 29 ] && [ ${stop_stage} -ge 29 ];then
   log " Stage 29 : show wer score for test set..."
