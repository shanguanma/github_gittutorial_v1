...
# This script computes the code-switching CER (Character Error Rate)
# score_kaldi_code-switching_cer.sh "$@"
...
cat $data/text | $ref_filtering_cmd > $dir/scoring_kaldi/test_filt.txt && wc $data/text && wc $dir/scoring_kaldi/test_filt.txt|| exit 1;
if [ $stage -le 0 ]; then
...
if [ $stage -le 2 ] ; then
  ...
  for f in "${files[@]}" ; do
    ...
    cat $f | PYTHONIOENCODING=utf-8 python3 source-md/analaysis/analysis_asr_results/scoring_character/segment-chinese-text.py --do-character-segmentation > $fout || exit 1;
      #./code-repository/asr-scripts/asr-kaldi-scripts/scroing/combine_abbreviations_kaldi_text.pl > $fout || exit 1;
      #> $fout || exit 1;
    sort $fout -o $fout
  done
  for wip in $(echo $word_ins_penalty | sed 's/,/ /g'); do
...
if [ $stage -le 3 ] ; then
  ...
  echo -e "\nBest is $best_cer_file:"
  cat $best_cer_file | grep -v '#' && echo
