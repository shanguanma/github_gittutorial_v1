...
while(<STDIN>) {
  ...
  if(IsNormalWavFile($wav_rspecifer) == 1) {  ## here normal wave file means a single wave file without pipe operation
    $wav_rspecifer =  "sox -t wav $wav_rspecifer -t wav -r $rate - trim $start_time $dur |";
  }elsif ($wav_rspecifer =~ m:trim\s(\d+): ){
    $wav_rspecifer =  "$wav_rspecifer";
  }else {  ## its a pipe-based specifier
    if ($wav_rspecifer =~ m:speed\s(\d+): ){
      $wav_rspecifer =~ s:\.wav -t wav - speed 0.9 \|$:\.wav -t wav -r $rate - speed 0.9 trim $start_time $dur \|:g;
      $wav_rspecifer =~ s:\.wav -t wav - speed 1.1 \|$:\.wav -t wav -r $rate - speed 1.1 trim $start_time $dur \|:g;
      $wav_rspecifer =~ s:\.WAV -t wav - speed 0.9 \|$:\.WAV -t wav -r $rate - speed 0.9 trim $start_time $dur \|:g;
      $wav_rspecifer =~ s:\.WAV -t wav - speed 1.1 \|$:\.WAV -t wav -r $rate - speed 1.1 trim $start_time $dur \|:g;
      $wav_rspecifer =~ s:\s+\|\s+sox -t wav - -t wav - speed 0.9 \|$: \| sox -t wav - -t wav -r $rate - speed 0.9 trim $start_time $dur \|:g;
      $wav_rspecifer =~ s:\s+\|\s+sox -t wav - -t wav - speed 1.1 \|$: \| sox -t wav - -t wav -r $rate - speed 1.1 trim $start_time $dur \|:g;
      $wav_rspecifer =~ s:-t\s+wav\s+-\s+\|\s+sox -t wav - -t wav -r $rate - speed:-t wav -r $rate - speed:g;
    }else{
      if ($wav_rspecifer =~ m:sox: ){
        $wav_rspecifer =~ s:-t\s+wav\s+-\s+\|$:-t wav -r $rate - trim $start_time $dur \|:g;
      }else{
        $wav_rspecifer = "$wav_rspecifer sox -t wav - -t wav -r $rate - trim $start_time $dur |";
      }
    }
  }
